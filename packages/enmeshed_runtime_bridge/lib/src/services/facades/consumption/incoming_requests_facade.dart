...
class IncomingRequestsFacade {
  ...
  Future<Result<RequestValidationResultDTO>> canAccept({required DecideRequestParameters params}) async {
    final result = await _evaluator.evaluateJavaScript(
      '''const result = await session.consumptionServices.incomingRequests.canAccept(request)
      if (result.isError) return { error: { message: result.error.message, code: result.error.code } }
      return { value: result.value }''',
      arguments: {'request': params.toJson()},
    );
  ...
  Future<Result<LocalRequestDTO>> accept({required DecideRequestParameters params}) async {
    final result = await _evaluator.evaluateJavaScript(
  ...
  Future<Result<RequestValidationResultDTO>> canReject({required DecideRequestParameters params}) async {
    final result = await _evaluator.evaluateJavaScript(
  ...
  Future<Result<LocalRequestDTO>> reject({required DecideRequestParameters params}) async {
    final result = await _evaluator.evaluateJavaScript(
  ...
  Future<Result<LocalRequestDTO>> getRequest({required String requestId}) async {
    final result = await _evaluator.evaluateJavaScript(
